...
import { getAllArticles, getAllSections } from "@/lib/reverts";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const found = getAllArticles().find(({ article: a }) => a.slug === article);
  if (!found) return {};
  ...
  return {
    ...
    alternates: {
      canonical: siteUrl(locale === "en" ? `/reverts/${article}` : `/${locale}/reverts/${article}`),
      languages: hreflangLanguages(`/reverts/${article}`),
    },
    openGraph: {
      url: siteUrl(locale === "en" ? `/reverts/${article}` : `/${locale}/reverts/${article}`),
    },
  };
}

/**
 * Render a body string that contains paragraphs separated by "\n\n" and
 * `**bold**` runs, without pulling in a full markdown library. Keeps the
 * rendered output pure server-side and small.
 */
function renderParagraphs(text: string): React.ReactNode[] {
  return text.split(/\n\n+/).map((para, i) => {
    // Split on **bold** markers to build up children.
...
export default async function RevertsArticlePage({ params }: Props) {
  const { locale, article } = await params;
  const found = getAllArticles().find(({ article: a }) => a.slug === article);
  if (!found) notFound();
  setRequestLocale(locale);
  const t = await getTranslations({ locale, namespace: "reverts.article" });
  const bc = await breadcrumbs(locale);
  const lang = (locale === "id" ? "id" : "en") as "en" | "id";

  // Determine prev / next across all articles (flat order).
    ...
    <article className="mx-auto max-w-reading px-4 sm:px-6 lg:px-8 py-12 md:py-16">
      <BreadcrumbSchema
        items={[
          ...
          { name: t("reverts"), url: siteUrl("/reverts") },
          { name: found.section.title[lang], url: siteUrl(`/reverts#section-${found.section.slug}`) },
          { name: found.article.title[lang], url: siteUrl(`/reverts/${found.article.slug}`) },
        ]}
      />
      <ArticleSchema
        headline={found.article.title[lang]}
        description={found.article.body[lang].split("\n")[0] ?? ""}
        url={siteUrl(`/reverts/${found.article.slug}`)}
      ...
      {found.article.linksTo.length > 0 && (
        <section aria-labelledby="links-heading" className="mt-10 rounded-2xl border border-separator bg-surface p-5">
          <h2 id="links-heading" className="text-sm font-semibold tracking-title">
            {t("keepReadingHeading")}
          </h2>
          <ul className="mt-3 space-y-2">
            {found.article.linksTo.map((link) => (
              <li key={link.href}>
                <Link
                  href={link.href as "/quran"}
      ...
      {found.article.references.length > 0 && (
        <section aria-labelledby="refs-heading" className="mt-8 rounded-2xl border border-separator bg-surface p-5">
      ...
      >
        <div>
          {prev ? (
            <Link
              href={`/reverts/${prev.article.slug}` as "/reverts/[article]"}
        ...
        <div>
          {next ? (
            <Link
              href={`/reverts/${next.article.slug}` as "/reverts/[article]"}
