...
import { getAllArticles, getArticle } from "@/lib/ramadan";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: siteUrl(locale === "en" ? `/ramadan/${article}` : `/${locale}/ramadan/${article}`),
      languages: hreflangLanguages(`/ramadan/${article}`),
    },
    openGraph: {
      url: siteUrl(locale === "en" ? `/ramadan/${article}` : `/${locale}/ramadan/${article}`),
    },
  };
}
...
export default async function RamadanArticlePage({ params }: Props) {
  const { locale, article } = await params;
  const found = getArticle(article);
  if (!found) notFound();
  setRequestLocale(locale);
  const t = await getTranslations({ locale, namespace: "ramadan.article" });
    ...
    <article className="mx-auto max-w-reading px-4 sm:px-6 lg:px-8 py-12 md:py-16">
      <BreadcrumbSchema
        items={[
          ...
          { name: t("ramadan"), url: siteUrl("/ramadan") },
          {
            name: found.section.title[lang],
            url: siteUrl(`/ramadan#section-${found.section.slug}`),
          },
          { name: found.article.title[lang], url: siteUrl(`/ramadan/${found.article.slug}`) },
        ]}
      />
      <ArticleSchema
        headline={found.article.title[lang]}
        description={found.article.body[lang].split("\n")[0] ?? ""}
        url={siteUrl(`/ramadan/${found.article.slug}`)}
        datePublished="2026-09-19"
      />

      <Link href="/ramadan" className="focus-ring text-sm text-accent hover:underline">
        {t("backToHub")}
      </Link>
      ...
      >
        <div>
          {prev ? (
            <Link
              href={`/ramadan/${prev.article.slug}` as "/ramadan/[article]"}
        ...
        <div>
          {next ? (
            <Link
              href={`/ramadan/${next.article.slug}` as "/ramadan/[article]"}
